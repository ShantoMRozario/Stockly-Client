...
import { deletePurchaseRequest, purchaseListRequest } from "../../Api/PurchaseApiRequest";
...
const PurchaseList = () => {
  ...
  useEffect(() => {
    purchaseListRequest(1, perPage, search);
  }, [currentPage, perPage, search]);

  const allPurchase = useSelector((state) => state.purchase.purchases);
  const total = useSelector((state) => state.purchase?.list[0]?.total);
  ...
  const handlePageChange = (pageNumber) => {
    ...
    purchaseListRequest(pageNumber, perPage, search);
  };
  ...
  const handlePerPage = (e) => {
    ...
    setCurrentPage(1);
    purchaseListRequest(1, parseInt(e.target.value), search);
  };
  ...
  const searchInput = (e) => {
    ...
    if ((e.target.value).length === 0) {
      setSearch(null);
      purchaseListRequest(currentPage, perPage, "null");
    }
  };
  ...
  const deletePurchase = async (id) => {
    ...
    return (
      ...
      <div className="flex  items-center w-full">
        <h1 className="text-3xl font-bold mb-5 w-[20%]">Purchase List</h1>
        <Link to='/purchase-create-update' className="btn bg-green-500 hover:bg-green-700 text-white font-bold "><MdOutlineCreate />Create New Purchase</Link>
          ...
          <tbody>
            {allPurchase?.map((item, index) => (
              <tr key={index} className="text-center hover:bg-green-100 ">
                ...
                <td className="px-5 py-2">{item.supplierData[0].supplierName}</td>
