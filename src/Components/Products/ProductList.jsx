...
import { deleteProductRequest, productListRequest } from "../../Api/ProductApiRequest";
...
const ProductList = () => {
    ...
    const total = useSelector((state) => state.product?.list[0]?.total);
    ...
    const handlePageChange = (pageNumber ) => {
      ...
      productListRequest(pageNumber, perPage, search);
    };
    ...
    const handlePerPage = (e) => {
      ...
      setCurrentPage(1);
      productListRequest(1,parseInt(e.target.value), search);
    };
    ...
    const searchInput = (e) => {
      ...
      if ((e.target.value).length === 0) {
        setSearch(null);
        productListRequest(1, perPage, "null");
      }
    };
    ...
    return (
      ...
      <div className="flex  items-center w-full">
        <h1 className="text-3xl font-bold mb-5 w-[20%]">Product List</h1>
        <Link to='/product-create-update' className="btn bg-green-500 hover:bg-green-700 text-white font-bold "><MdOutlineCreate />Create New Product </Link>
          ...
          <tbody>
            {allProducts?.map((item, index) => (
              <tr key={index} className="text-center hover:bg-green-100 ">
                ...
                <td className="px-5 py-2 ">{item.productName}</td>
                <td className="px-5 py-2 ">
                    <img className="w-[100px] mx-auto" src={item.images[0]} alt="" />
                </td>
                <td className="px-5 py-2 ">{item.brandData[0].name}</td>
                <td className="px-5 py-2 ">{item.categoryData[0].name}</td>
                ...
                <td className="flex gap-3 justify-center">
                  <Link
                    to={`/product-create-update/${item._id}`}
