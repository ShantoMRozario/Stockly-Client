...
import { categoryListRequest, deleteCategoryRequest } from "../../Api/CategoryApiRequest";
...
const CategoryList = () => {
    ...
    const allCategory = useSelector((state) => state.category?.categorys);
    const total = useSelector((state) => state.category?.list[0]?.total);
    ...
    const handlePageChange = (pageNumber ) => {
      ...
      categoryListRequest(pageNumber, perPage, search);
    };
    ...
    const handlePerPage = (e) => {
      ...
      setCurrentPage(1);
      categoryListRequest(1, parseInt(e.target.value), search);
    };
    ...
    const searchInput = (e) => {
      ...
      if ((e.target.value).length === 0) {
        setSearch(null);
        categoryListRequest(1, perPage, "null");
      }
    };
    ...
    return (
        <div>
          <div className="flex  items-center w-full">
            <h1 className="text-3xl font-bold mb-5 w-[20%]">Category List</h1>
            <Link to='/category-create-update' className="btn bg-green-500 hover:bg-green-700 text-white font-bold "><MdOutlineCreate />Create Category</Link>
            ...
            <table className="table table-sm table-pin-rows table-pin-cols">
              <thead>
                <tr className="text-center bg-red-500">
                  ...
                  <th className="px-5 py-2 ">Category Name</th>
                ...
                {allCategory?.map((item, index) => (
                  <tr key={index} className="text-center hover:bg-green-100 ">
                    ...
                    <td className="flex gap-3 justify-center">
                      <Link
                        to={`/category-create-update/${item._id}`}
