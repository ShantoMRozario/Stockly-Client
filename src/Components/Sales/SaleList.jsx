...
import { deleteSalesRequest, salesListRequest } from "../../Api/SalesApiRequest";
...
const SaleList = () => {
  ...
  const total = useSelector((state) => state.sale?.list[0]?.total);
  ...
  const handlePageChange = (pageNumber) => {
    ...
    salesListRequest(pageNumber, perPage, search);
  };
  ...
  const handlePerPage = (e) => {
    ...
    setCurrentPage(1);
    salesListRequest(1, parseInt(e.target.value), search);
  };
  ...
  const searchInput = (e) => {
    ...
    if ((e.target.value).length === 0) {
      setSearch(null);
      salesListRequest(currentPage, perPage, "null");
    }
  };
  ...
  const deleteSales = async (id) => {
    ...
    return (
        ...
        <div className="flex  items-center w-full">
          <h1 className="text-3xl font-bold mb-5 w-[20%]">Sales List</h1>
          <Link to='/sale-create-update' className="btn bg-green-500 hover:bg-green-700 text-white font-bold "><MdOutlineCreate />Create New Sales</Link>
