...
import { returnReportRequest } from '../../Api/ReportApiRequest';
...
const ReturnReport = () => {
  ...
  const returnReport  = useSelector(state => state.report.returnsReportByDate);
  console.log(returnReport);
  ...
  const handleCreateReport = async() => {
    ...
    if(fromDate && toDate){
        let result = await returnReportRequest(fromDate, toDate)
        if(result == true){
          setLoader(false);
          setError(true);
        }
  ...
  const handleDownload = (downloadType,data) => {
    if(data.length > 0){
      ...
      data.map((item) => {
        let list = {
          'product': item.productData.productName,
          'brand': item.brandData[0].name,
          'category': item.categoryData[0].name, 
          'unit': item.productData.unit,
          'unitPrice': item.unitPrice,
          'Total': item.total,
          'details': item.productData.details,
    ...
    return (
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-2xl border-2 border-gray-100">
        <h2 className="text-2xl font-bold mb-6 text-center">Return Report By Date</h2>
        ...
        {
          returnReport[0]?.data?.length > 0 ?
          <div className="mt-6">
                <div className="bg-purple-100 p-4 rounded-lg">
                  <h3 className="text-xl font-bold">Download Report</h3>
                  <p className='text-lg'>Total: $ {returnReport[0]?.total[0].totalAmount}</p>
                  <button onClick={() => handleDownload('xls',returnReport[0]?.data)} className="mt-4 bg-green-500 text-white mx-2 py-2 px-4 rounded-lg hover:bg-green-600 transition duration-200">Download Excel</button>
                  <button onClick={() => handleDownload('txt',returnReport[0]?.data)} className="mt-4 bg-gray-400 text-white mx-2 py-2 px-4 rounded-lg hover:bg-gray-500 transition duration-200">Download Text</button>
                  <button onClick={() => handleDownload('csv',returnReport[0]?.data)} className="mt-4 bg-purple-400 text-white mx-2 py-2 px-4 rounded-lg hover:bg-purple-600 transition duration-200">Download CSV</button>
                </div>
          </div>
          :
          error &&
