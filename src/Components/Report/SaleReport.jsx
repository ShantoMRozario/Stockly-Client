...
import { saleReportRequest } from '../../Api/ReportApiRequest';
...
const SaleReport = () => {
    ...
    const salesReport  = useSelector(state => state.report.salesReportByDate);
    ...
    const handleCreateReport = async() => {
      ...
      if(fromDate && toDate){
          let result = await saleReportRequest(fromDate, toDate)
          if(result == true){
            setLoader(false);
            setError(true);
          }
    ...
    return (
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-2xl border-2 border-gray-100">
        <h2 className="text-2xl font-bold mb-6 text-center">Sales Report By Date</h2>
        ...
        {
          salesReport[0]?.data?.length > 0 ?
          <div className="mt-6">
                <div className="bg-purple-100 p-4 rounded-lg">
                  <h3 className="text-xl font-bold">Download Report</h3>
                  <p className='text-lg'>Total: $ {salesReport[0]?.total[0].totalAmount}</p>
                  <button onClick={() => handleDownload('xls',salesReport[0]?.data)} className="mt-4 bg-green-500 text-white mx-2 py-2 px-4 rounded-lg hover:bg-green-600 transition duration-200">Download Excel</button>
                  <button onClick={() => handleDownload('txt',salesReport[0]?.data)} className="mt-4 bg-gray-400 text-white mx-2 py-2 px-4 rounded-lg hover:bg-gray-500 transition duration-200">Download Text</button>
                  <button onClick={() => handleDownload('csv',salesReport[0]?.data)} className="mt-4 bg-purple-400 text-white mx-2 py-2 px-4 rounded-lg hover:bg-purple-600 transition duration-200">Download CSV</button>
                </div>
          </div>
          :
          error &&
